 ...
 */
void
zbc_write_native_uint(uint8_t *buf, uintptr_t value, int size, int endianness)
{
    int i;
 ...
 */
intptr_t
zbc_read_native_int(const uint8_t *buf, int size, int endianness)
{
    uintptr_t value = 0;
    int       i;
    uintptr_t sign_bit;
 ...
 */
uintptr_t
zbc_read_native_uint(const uint8_t *buf, int size, int endianness)
{
    uintptr_t value = 0;
    int       i;
 ...
 */
uint8_t *
zbc_riff_begin_chunk(uint8_t *buf, size_t capacity, size_t *offset, uint32_t fourcc)
{
    uint8_t *size_ptr;
 ...
 */
void
zbc_riff_patch_size(uint8_t *size_ptr, size_t data_size)
{
 ...
 */
int
zbc_riff_write_bytes(uint8_t *buf, size_t capacity, size_t *offset, const void *data, size_t size)
{
    size_t         i;
    const uint8_t *src;
 ...
 */
void
zbc_riff_pad(uint8_t *buf, size_t capacity, size_t *offset)
{
 ...
 */
int
zbc_riff_read_header(const uint8_t *buf, size_t capacity, size_t offset, uint32_t *fourcc,
                     uint32_t *size)
{
    if (offset + ZBC_CHUNK_HDR_SIZE > capacity) {
 ...
 */
size_t
zbc_riff_skip_chunk(const uint8_t *buf, size_t capacity, size_t offset)
{
 ...
 */
uint8_t *
zbc_riff_begin_container(uint8_t *buf, size_t capacity, size_t *offset, uint32_t form_type)
{
    uint8_t *size_ptr;
 ...
 */
int
zbc_riff_validate_container(const uint8_t *buf, size_t capacity, uint32_t expected_form_type)
{
...
int
zbc_chunk_validate(const zbc_chunk_t *chunk, const uint8_t *container_end)
{
...
int
zbc_chunk_next(zbc_chunk_t **out, const zbc_chunk_t *chunk)
{
...
int
zbc_chunk_first_sub(zbc_chunk_t **out, const zbc_chunk_t *container, size_t header_size)
{
    if (!out || !container) {
        return ZBC_ERR_NULL_ARG;
    }
...
int
zbc_chunk_end(const uint8_t **out, const zbc_chunk_t *chunk)
{
    if (!out || !chunk) {
        return ZBC_ERR_NULL_ARG;
    }
...
int
zbc_chunk_find(zbc_chunk_t **out, const uint8_t *start, const uint8_t *end, uint32_t id)
{
    zbc_chunk_t *chunk;
    int          err;
...
int
zbc_riff_validate(const zbc_riff_t *riff, size_t buf_size, uint32_t expected_form)
{
    ...
    if (riff->form_type != expected_form) {
        ZBC_LOG_ERROR("bad form type: 0x%08x (expected 0x%08x)", (unsigned)riff->form_type,
                      (unsigned)expected_form);
        return ZBC_ERR_BAD_FORM_TYPE;
    }
    ...
    if (riff_total_size > buf_size) {
        ZBC_LOG_ERROR("RIFF overflow: size=%u exceeds buf_size=%u", (unsigned)riff->size,
                      (unsigned)buf_size);
...
int
zbc_riff_end(const uint8_t **out, const zbc_riff_t *riff)
{
    if (!out || !riff) {
        return ZBC_ERR_NULL_ARG;
    }
 ...
 */
static void
parse_subchunks(const uint8_t *start, const uint8_t *end, int int_size, int ptr_size, int endian,
                zbc_parsed_t *out)
{
 ...
 */
int
zbc_riff_parse_request(zbc_parsed_t *out, const uint8_t *buf, size_t buf_size, int int_size,
                       int endian)
{
    ...
    while (pos + ZBC_CHUNK_HDR_SIZE <= riff_end_ptr) {
        ...
        if (chunk_data + size > riff_end_ptr) {
            ZBC_LOG_ERROR("chunk data overflow: size=%u exceeds container", (unsigned)size);
            return ZBC_ERR_DATA_OVERFLOW;
        }
        ...
        if (id == ZBC_ID_CNFG) {
            ...
            if (size >= ZBC_CNFG_PAYLOAD_SIZE) {
                ...
                if (out->int_size > sizeof(uintptr_t) || out->ptr_size > sizeof(uintptr_t)) {
            ...
            if (size >= ZBC_CALL_HDR_PAYLOAD_SIZE) {
                ...
                parse_subchunks(chunk_data + ZBC_CALL_HDR_PAYLOAD_SIZE, chunk_data + size, int_size,
                                ptr_size, endian, out);
            }
            ...
            if (size >= (size_t)int_size + ZBC_RETN_ERRNO_SIZE) {
                ...
                parse_subchunks(chunk_data + int_size + ZBC_RETN_ERRNO_SIZE, chunk_data + size,
