...
void
zbc_client_init(zbc_client_state_t *state, volatile void *dev_base)
{
    if (!state) {
        return;
    }
...
int
zbc_client_check_signature(const zbc_client_state_t *state)
{
...
void
zbc_client_reset_cnfg(zbc_client_state_t *state)
{
    if (state) {
        state->cnfg_sent = 0;
    }
 ...
 */
static size_t
calculate_retn_capacity(const zbc_opcode_entry_t *entry, const zbc_client_state_t *state,
                        uintptr_t *args)
{
    size_t base_size;
    size_t data_len;
    ...
    case ZBC_RESP_DATA:
        ...
        return base_size + ZBC_CHUNK_HDR_SIZE + ZBC_DATA_HDR_SIZE + ZBC_PAD_SIZE(data_len);
 ...
 */
static int
write_cnfg_if_needed(uint8_t *buf, size_t capacity, size_t *pos, zbc_client_state_t *state)
{
    size_t   total_size;
    uint8_t *payload;
    ...
    if (*pos + total_size > capacity) {
        ZBC_LOG_ERROR("write_cnfg: buffer full (need %u, have %u)", (unsigned)(*pos + total_size),
                      (unsigned)capacity);
 ...
 */
static int
write_parm_chunk(uint8_t *buf, size_t capacity, size_t *pos, uintptr_t value, int is_signed,
                 const zbc_client_state_t *state)
{
    ...
    payload[2] = 0;
    payload[3] = 0;
    zbc_write_native_uint(payload + 4, (uint64_t)value, state->int_size, state->endianness);
 ...
 */
static int
write_data_chunk(uint8_t *buf, size_t capacity, size_t *pos, const void *data, size_t len,
                 int data_type)
{
    size_t         payload_size;
    size_t         total_size;
 ...
 */
static int
build_request(uint8_t *buf, size_t capacity, size_t *out_size, zbc_client_state_t *state,
              const zbc_opcode_entry_t *entry, uintptr_t *args)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        case ZBC_CHUNK_DATA_STR: {
            ...
            rc = write_data_chunk(buf, capacity, &pos, str, len, ZBC_DATA_TYPE_STRING);
        } break;

        case ZBC_CHUNK_DATA_BYTE: {
            uint8_t byte = *(uint8_t *)args[desc->slot];
            rc = write_data_chunk(buf, capacity, &pos, &byte, 1, ZBC_DATA_TYPE_BINARY);
        } break;
...
int
zbc_client_submit(zbc_client_state_t *state, void *buf, size_t size)
{
...
int
zbc_parse_response(zbc_response_t *response, const uint8_t *buf, size_t capacity,
                   const zbc_client_state_t *state)
{
...
int
zbc_call(zbc_response_t *response, zbc_client_state_t *state, void *buf, size_t buf_size,
...
uintptr_t
zbc_semihost(zbc_client_state_t *state, uint8_t *riff_buf, size_t riff_buf_size, uintptr_t op,
             uintptr_t param)
{
